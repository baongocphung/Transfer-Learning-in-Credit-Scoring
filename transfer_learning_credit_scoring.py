...
#
# NOTE QUAN TRỌNG [Deviation từ Ganin 2016 gốc]:
#   Ganin 2016 là UNSUPERVISED DA - không dùng nhãn target. Code này dùng
#   L_label_T (semi-supervised), phù hợp với context credit scoring nơi
#   DCCC có nhãn. Đây là biến thể Bousmalis 2017 NeurIPS.
#
# ════════════════════════════════════════════════════════════════════════
...
def train_dann(model, X_s, y_s, X_t, y_t,
    ...
    NOTE [Loss tổng]:
        L = BCE_s(pos_weight=w_s)
          + gamma_t * BCE_t(pos_weight=w_t)
          + BCE_d  (chú ý: GRL nằm GIỮA encoder và domain classifier
                    nên gradient của BCE_d lan xuống encoder bị đảo
                    dấu và scale theo lambda)

    NOTE [Class weighting]:
        - pos_weight_S = (#neg_S) / (#pos_S) ≈ 6.14 (LC 14% default).
        - pos_weight_T = (#neg_T) / (#pos_T) ≈ 3.52 (DCCC 22% default).
        - gamma_t = sqrt(n_s/n_t) ≈ 3.16 — boost target signal khi
          n_s >> n_t (Cao 2019 NeurIPS, label-distribution-aware loss).
        ⇒ Ba cơ chế bù imbalanced + source dominance ⇒ tránh NT.
