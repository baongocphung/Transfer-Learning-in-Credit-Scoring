"""
================================================================================
 TRANSFER LEARNING CHO CREDIT SCORING — v10
 Source: Lending Club →  Target: Default of Credit Card Clients
 [UPDATE v10]: 2 trường hợp (TH) cho LC features
================================================================================
...
[UPDATE v10] Sau tiền xử lý LC, chia thành 2 trường hợp:
  TH1: Full features + improved imputation (thay -99999 → median + indicator)
  TH2: Selected features (chọn lọc theo 5 C's of Credit, overlap DCCC)
  → Cả HFA (Phần 2) và DANN (Phần 3) đều chạy cho cả TH1 và TH2.
  ...
  PHẦN 2 — HFA (Heterogeneous Feature Augmentation, Yang 2020 Eq. 6.8)
    ...
    2.2 Tiền xử lý LC (loại post-loan, imputation, encode, subsample 300K)
        2.2.10 [NEW] Tạo 2 trường hợp TH1 + TH2
        2.2.11 [NEW] Scale cho 2 TH
    2.3 HFA core
    ...
    2.4–2.6 [LOOP TH1, TH2] 5-fold CV + Hold-out + 3-case Ablation

  PHẦN 3 — DANN (Domain-Adversarial Neural Network, Ganin 2016)
    3.1 Kiến trúc (giữ nguyên v9)
    3.2 Lịch trình
    3.3 train_dann()
    3.4 predict_dann()
    3.5–3.7 [LOOP TH1, TH2] 5-fold CV + Hold-out + 3-case Ablation

  FINAL — So sánh Baseline / HFA / DANN × TH1 / TH2
"""
...
# ── 2.2.10. Tạo 2 trường hợp (TH) cho LC features ──────────────────────
# ┌─────────────────────────────────────────────────────────────────────────┐
# │  TH1: Full features + Improved imputation                              │
# │       - Giữ nguyên TẤT CẢ d_s cột đã qua OHE/label encode.            │
# │       - Thay sentinel -99999 (từ bước 2.2.4 & 2.2.6) bằng MEDIAN      │
# │         tính trên giá trị hợp lệ + thêm cột binary _was_missing       │
# │         để giữ thông tin missingness (Enders 2010 Ch.2; Josse 2019).   │
# │       - Lý do: -99999 làm sai lệch phân phối, đặc biệt cho            │
# │         StandardScaler (outlier cực âm) và NN (DANN). Median+indicator │
# │         giữ trung tâm phân phối và cho model học pattern missing.       │
# │                                                                        │
# │  TH2: Selected features — chọn lọc theo overlap ngữ nghĩa với DCCC    │
# │       - Cơ sở: 5 C's of Credit (Altman & Saunders 1997) + đối chiếu   │
# │         trực tiếp từng nhóm feature LC (Report v2 §3.2 Table 1)        │
# │         với feature tương ứng của DCCC (Report v2 §3.1 Table 0).       │
# │       - Mục đích: giảm domain gap bằng cách chỉ giữ features có       │
# │         ngữ nghĩa tương đồng, giảm noise từ features không liên quan. │
# │       - Tham khảo: Pan & Yang (2010 §3.3) khuyến nghị giảm feature    │
# │         space khi source-target divergence cao để tránh Negative        │
# │         Transfer.                                                       │
# └─────────────────────────────────────────────────────────────────────────┘
LOG2.section("2.2.10. Tạo 2 trường hợp (TH) cho LC features", level=3)

# ─────────────────────────────────────────────────────────────────────────
# TH1: Full features + improved imputation (thay -99999)
# ─────────────────────────────────────────────────────────────────────────
LOG2.write("── TH1: Full features + Improved imputation ──")
LOG2.write("  Chiến lược: thay sentinel -99999 → median(cột, loại -99999)")
LOG2.write("  + thêm cột binary <tên>_was_missing (Enders 2010; Josse 2019).")

df_lc_th1 = df_lc.copy()
SENTINEL = -99999
sentinel_report = []

for c in [col for col in df_lc_th1.columns if col != "y"]:
    mask = (df_lc_th1[c] == SENTINEL)
    if mask.any():
        n_sent = int(mask.sum())
        valid_vals = df_lc_th1.loc[~mask, c]
        fill_val = float(valid_vals.median()) if len(valid_vals) > 0 else 0.0
        df_lc_th1.loc[mask, c] = fill_val
        df_lc_th1[f"{c}_was_missing"] = mask.astype(int)
        sentinel_report.append((c, n_sent, fill_val))

LOG2.write(f"  Số cột có sentinel: {len(sentinel_report)}")
for col_name, n_s, fv in sentinel_report:
    LOG2.write(f"    {col_name}: {n_s} giá trị -99999 → median={fv:.2f}, "
               f"+ cột {col_name}_was_missing")
LOG2.write(f"  TH1 shape sau thêm indicator: {df_lc_th1.shape}")

# ─────────────────────────────────────────────────────────────────────────
# TH2: Selected features — chọn lọc theo overlap ngữ nghĩa với DCCC
# ─────────────────────────────────────────────────────────────────────────
LOG2.write("")
LOG2.write("── TH2: Selected features (chọn lọc) ──")
LOG2.write("  Cơ sở chọn biến — 5 C's of Credit (Altman & Saunders 1997):")
LOG2.write("  ┌──────────────┬────────────────────────────┬─────────────────────────┐")
LOG2.write("  │ Tiêu chí     │ LC features chọn           │ DCCC features tương ứng │")
LOG2.write("  ├──────────────┼────────────────────────────┼─────────────────────────┤")
LOG2.write("  │ Character    │ delinq_2yrs, grade,        │ PAY_0..PAY_6            │")
LOG2.write("  │              │ sub_grade, cr_history_yr   │ (lịch sử thanh toán)    │")
LOG2.write("  ├──────────────┼────────────────────────────┼─────────────────────────┤")
LOG2.write("  │ Capacity     │ annual_inc, dti,           │ LIMIT_BAL, PAY_AMT1..6  │")
LOG2.write("  │              │ emp_length, installment    │ (khả năng trả nợ)       │")
LOG2.write("  ├──────────────┼────────────────────────────┼─────────────────────────┤")
LOG2.write("  │ Capital      │ loan_amnt, revol_util,     │ BILL_AMT1..6, LIMIT_BAL │")
LOG2.write("  │              │ open_acc, total_acc        │ (quy mô tín dụng)       │")
LOG2.write("  ├──────────────┼────────────────────────────┼─────────────────────────┤")
LOG2.write("  │ Collateral   │ home_ownership_*           │ MARRIAGE (nhân khẩu)    │")
LOG2.write("  ├──────────────┼────────────────────────────┼─────────────────────────┤")
LOG2.write("  │ Conditions   │ int_rate, term, purpose_*, │ EDUCATION (context)     │")
LOG2.write("  │              │ verification_status_*      │                         │")
LOG2.write("  └──────────────┴────────────────────────────┴─────────────────────────┘")
LOG2.write("  Thêm lý do:")
LOG2.write("    - Loại bỏ ~60-70% features LC không có đối ứng ngữ nghĩa trong DCCC")
LOG2.write("      (ví dụ: các biến về policy, collection, recovery, nhiều biến nhỏ lẻ)")
LOG2.write("    - Pan & Yang (2010 §3.3): giảm feature space khi domain gap cao")
LOG2.write("      có thể giúp tránh Negative Transfer.")
LOG2.write("    - Report v2 §3.2 Table 1 liệt kê các nhóm feature tiêu biểu LC.")

# Danh sách base features (trước OHE) chọn lọc
SELECTED_BASE_COLS = [
    # Character (tín nhiệm / lịch sử)
    "delinq_2yrs",       # delinquencies 2 năm → overlap PAY_X (DCCC)
    "grade",             # LC internal grade A-G → proxy rủi ro
    "sub_grade",         # chi tiết hơn grade (A1-G5)
    "cr_history_yr",     # số năm lịch sử tín dụng (derived từ earliest_cr_line)
    # Capacity (khả năng trả nợ)
    "annual_inc",        # thu nhập năm → khả năng chi trả (~ LIMIT_BAL)
    "dti",               # debt-to-income → leverage ratio
    "emp_length",        # thâm niên việc làm → ổn định thu nhập
    "installment",       # khoản trả hàng tháng (~ PAY_AMT)
    # Capital (quy mô tín dụng)
    "loan_amnt",         # số tiền vay (~ LIMIT_BAL)
    "revol_util",        # revolving utilization (~ BILL_AMT/LIMIT_BAL)
    "open_acc",          # số tài khoản đang mở
    "total_acc",         # tổng tài khoản từ trước đến nay
    # Conditions (điều kiện vay)
    "int_rate",          # lãi suất → phản ánh risk pricing
    "term",              # kỳ hạn 36/60 tháng
]

# OHE prefixes (Collateral + Conditions + context)
OHE_PREFIXES_SELECTED = [
    "home_ownership_",        # Collateral: nhà ở
    "verification_status_",   # Conditions: xác minh thu nhập
    "purpose_",               # Conditions: mục đích vay
    "region_",                # Conditions: vùng địa lý
]

# Build danh sách cột TH2
selected_cols = [c for c in SELECTED_BASE_COLS if c in df_lc.columns]
for prefix in OHE_PREFIXES_SELECTED:
    selected_cols += sorted([c for c in df_lc.columns
                              if c.startswith(prefix) and c not in selected_cols])
selected_cols = [c for c in selected_cols if c in df_lc.columns]

LOG2.write(f"  Số base features: {len([c for c in SELECTED_BASE_COLS if c in df_lc.columns])}")
LOG2.write(f"  Số OHE features: {len(selected_cols) - len([c for c in SELECTED_BASE_COLS if c in df_lc.columns])}")
LOG2.write(f"  Tổng features TH2: {len(selected_cols)}")
LOG2.write(f"  Danh sách: {selected_cols}")

df_lc_th2 = df_lc[selected_cols + ["y"]].copy()

# TH2 cũng thay -99999 (nếu có) bằng median
for c in [col for col in df_lc_th2.columns if col != "y"]:
    mask = (df_lc_th2[c] == SENTINEL)
    if mask.any():
        valid_vals = df_lc_th2.loc[~mask, c]
        fill_val = float(valid_vals.median()) if len(valid_vals) > 0 else 0.0
        df_lc_th2.loc[mask, c] = fill_val

LOG2.write(f"  TH2 shape: {df_lc_th2.shape}")

# ── 2.2.11. Tách features / nhãn + StandardScaler cho 2 TH ─────────────
LOG2.section("2.2.11. Tách features / nhãn + StandardScaler cho 2 TH", level=3)

lc_variants = OrderedDict()

for th_name, th_df, th_label in [
    ("TH1_full",     df_lc_th1,     "TH1 (Full features + improved imputation)"),
    ("TH2_selected", df_lc_th2,     "TH2 (Selected features)"),
]:
    y_th   = th_df["y"].values.astype(int)
    X_th   = th_df.drop(columns=["y"]).values.astype(float)
    feat_th = [c for c in th_df.columns if c != "y"]
    scaler_th  = StandardScaler().fit(X_th)
    X_th_scaled = scaler_th.transform(X_th)
    lc_variants[th_name] = {
        "X_raw":     X_th,
        "y":         y_th,
        "features":  feat_th,
        "X_scaled":  X_th_scaled,
        "scaler":    scaler_th,
        "label":     th_label,
    }
    LOG2.write(f"  {th_name}: d_s={X_th.shape[1]}, n_s={X_th.shape[0]}, "
               f"default rate={y_th.mean():.4f}")

# Target scaling (chung cho cả 2 TH — giống v9)
scaler_t    = StandardScaler().fit(X_dc_tr)
X_t_tr_scaled = scaler_t.transform(X_dc_tr)
X_t_te_scaled = scaler_t.transform(X_dc_te)
LOG2.write(f"  Target: d_t={X_dc_tr.shape[1]}, n_train={X_dc_tr.shape[0]}, "
           f"n_test={X_dc_te.shape[0]}")
...
# ── Helper: SVM fit-predict cho ablation cases ──────────────────────────────
def _svm_fit_predict(X_tr, y_tr, X_te):
    """Fit LinearSVC trên X_tr, predict probability trên X_te via Platt scaling."""
    # dual=False: n_samples (300K+) >> n_features -> primal nhanh hon
...
# ──────────────────────────────────────────────────────────────────────────────
# 2.4–2.6. HFA — LOOP trên 2 TH (TH1_full, TH2_selected)
# ──────────────────────────────────────────────────────────────────────────────
LOG2.section("2.4–2.6. HFA — Chạy cho cả TH1 và TH2", level=2)

hfa_all_results = OrderedDict()   # {th_name: {cv, holdout, ablation, model}}

for th_name, th_data in lc_variants.items():
    th_label = th_data["label"]
    X_s_scaled = th_data["X_scaled"]
    y_lc       = th_data["y"]
    d_s_th     = X_s_scaled.shape[1]

    LOG2.section(f"═══ HFA — {th_label} (d_s={d_s_th}) ═══", level=2)

    # ── 2.4. 5-fold CV ──────────────────────────────────────────────────
    LOG2.section(f"2.4. HFA — 5-fold CV [{th_name}]", level=2)
    log_cv_path = OUTPUT_DIR / f"02a_hfa_cv_{th_name}.txt"
    LOG_CV2 = TeeLogger(log_cv_path)
    LOG_CV2.section(f"PHẦN 2 — HFA — 5-FOLD CV [{th_label}]", level=1)
    ...
    LOG_CV2.close()
    LOG2.write(f"Trung bình 5-fold [{th_name}]: AUC={auc_mean:.4f}±{auc_std:.4f}, "
               f"KS={ks_mean:.4f}±{ks_std:.4f}")
    LOG2.write(f"Chi tiết → {log_cv_path.name}")

    # ── 2.5. Hold-out + 3-case Ablation ─────────────────────────────────
    LOG2.section(f"2.5. HFA — Hold-out + 3-case Ablation [{th_name}]", level=2)
    log_ho_path = OUTPUT_DIR / f"02b_hfa_holdout_{th_name}.txt"
    log_ab_path = OUTPUT_DIR / f"02c_hfa_ablation_{th_name}.txt"
    LOG_HO2 = TeeLogger(log_ho_path)
    LOG_HO2.section(f"PHẦN 2 — HFA — HOLD-OUT [{th_label}]", level=1)
    LOG_AB2 = TeeLogger(log_ab_path)
    LOG_AB2.section(f"PHẦN 2 — HFA — 3-CASE ABLATION [{th_label}]", level=1)
    ...
    hdr_ab = f"  {'Case':<25s} {'AUC':>8s} {'KS':>8s} {'Brier':>8s} {'Sens':>8s} {'Spec':>8s}"
    LOG_AB2.section("Bảng tổng hợp 3-case ablation", level=2)
    LOG_AB2.write(hdr_ab); LOG_AB2.write("  " + "-" * 70)
    for r in ablation_results:
        LOG_AB2.write(f"  {r['Model']:<25s} {r['AUC']:>8.4f} {r['KS']:>8.4f} "
                      f"{r['Brier']:>8.4f} {r['Sens']:>8.4f} {r['Spec']:>8.4f}")
    LOG_AB2.close(); LOG_HO2.close()
    LOG2.write(f"Chi tiết hold-out → {log_ho_path.name}")
    LOG2.write(f"Chi tiết ablation → {log_ab_path.name}")

    # ── 2.6. Tóm tắt ────────────────────────────────────────────────────
    LOG2.section(f"2.6. Tóm tắt HFA [{th_name}]", level=2)
    LOG2.write(f"5-fold CV: AUC={auc_mean:.4f}±{auc_std:.4f}, "
               f"KS={ks_mean:.4f}±{ks_std:.4f}")
    LOG2.write(f"Hold-out 3-case [{th_name}]:")
    for r in ablation_results:
        LOG2.write(f"  {r['Model']:<25s} | {fmt_metrics_row(r)}")

    hfa_all_results[th_name] = {
        "cv_scores":   hfa_cv_scores,
        "cv_auc_mean": auc_mean, "cv_auc_std": auc_std,
        "cv_ks_mean":  ks_mean,  "cv_ks_std":  ks_std,
        "ablation":    ablation_results,
        "holdout_main": ablation_results[2],   # Case 3 = HFA full
        "model":       hfa_model,
        "X_s_cv":      X_s_cv,
        "y_s_cv":      y_s_cv,
    }
...
LOG2.close()
RUN_LOG.write(f"[Done] PHẦN 2 — HFA (TH1 + TH2) → {OUTPUT_DIR}/02_hfa.txt")
...
# ── Helper: train target-only (dùng cho Case 1 ablation) ────────────────────
def _train_target_only(model, X_t, y_t, total_steps=4000, batch_size=64, lr=0.01):
    """Train chỉ trên target (encoder_t + label), không adversarial."""
...
# ──────────────────────────────────────────────────────────────────────────────
# 3.5–3.7. DANN — LOOP trên 2 TH (TH1_full, TH2_selected)
# ──────────────────────────────────────────────────────────────────────────────
LOG3.section("3.5–3.7. DANN — Chạy cho cả TH1 và TH2", level=2)

dann_all_results = OrderedDict()   # {th_name: {cv, holdout, ablation}}

for th_name, th_data in lc_variants.items():
    th_label = th_data["label"]
    # Lấy X_s_cv, y_s_cv đã subsample từ HFA (cùng seed, cùng index)
    X_s_cv = hfa_all_results[th_name]["X_s_cv"]
    y_s_cv = hfa_all_results[th_name]["y_s_cv"]
    d_s_th = X_s_cv.shape[1]

    LOG3.section(f"═══ DANN — {th_label} (d_s={d_s_th}) ═══", level=2)

    # ── 3.5. 5-fold CV ──────────────────────────────────────────────────
    LOG3.section(f"3.5. DANN — 5-fold CV [{th_name}]", level=2)
    log_cv_path = OUTPUT_DIR / f"03a_dann_cv_{th_name}.txt"
    LOG_CV3 = TeeLogger(log_cv_path)
    LOG_CV3.section(f"PHẦN 3 — DANN — 5-FOLD CV [{th_label}]", level=1)
    ...
    LOG_CV3.close()
    LOG3.write(f"Trung bình 5-fold [{th_name}]: AUC={dann_auc_mean:.4f}±{dann_auc_std:.4f}, "
               f"KS={dann_ks_mean:.4f}±{dann_ks_std:.4f}")
    LOG3.write(f"Chi tiết → {log_cv_path.name}")

    # ── 3.6. Hold-out + 3-case Ablation ─────────────────────────────────
    LOG3.section(f"3.6. DANN — Hold-out + 3-case Ablation [{th_name}]", level=2)
    log_ho_path = OUTPUT_DIR / f"03b_dann_holdout_{th_name}.txt"
    log_ab_path = OUTPUT_DIR / f"03c_dann_ablation_{th_name}.txt"
    LOG_HO3 = TeeLogger(log_ho_path)
    LOG_HO3.section(f"PHẦN 3 — DANN — HOLD-OUT [{th_label}]", level=1)
    LOG_AB3 = TeeLogger(log_ab_path)
    LOG_AB3.section(f"PHẦN 3 — DANN — 3-CASE ABLATION [{th_label}]", level=1)

    dann_ablation = []

    # --- Case 1: DCCC alone (encoder_t only, λ=0)
    LOG_AB3.section("Case 1: DCCC alone (chỉ encoder_t, λ=0)", level=2)
    model = HeteroDANN(d_s=X_s_cv.shape[1], d_t=X_t_tr_scaled.shape[1],
                       hidden=128, p_drop=0.2)
    ...
    LOG_AB3.write(hdr_ab); LOG_AB3.write("  " + "-" * 70)
    for r in dann_ablation:
        LOG_AB3.write(f"  {r['Model']:<25s} {r['AUC']:>8.4f} {r['KS']:>8.4f} "
                      f"{r['Brier']:>8.4f} {r['Sens']:>8.4f} {r['Spec']:>8.4f}")
    LOG_AB3.close(); LOG_HO3.close()
    LOG3.write(f"Chi tiết hold-out → {log_ho_path.name}")
    LOG3.write(f"Chi tiết ablation → {log_ab_path.name}")

    # ── 3.7. Tóm tắt ────────────────────────────────────────────────────
    LOG3.section(f"3.7. Tóm tắt DANN [{th_name}]", level=2)
    LOG3.write(f"5-fold CV: AUC={dann_auc_mean:.4f}±{dann_auc_std:.4f}, "
               f"KS={dann_ks_mean:.4f}±{dann_ks_std:.4f}")
    LOG3.write(f"Hold-out 3-case [{th_name}]:")
    for r in dann_ablation:
        LOG3.write(f"  {r['Model']:<25s} | {fmt_metrics_row(r)}")

    dann_all_results[th_name] = {
        "cv_scores":    dann_cv_scores,
        "cv_auc_mean":  dann_auc_mean, "cv_auc_std": dann_auc_std,
        "cv_ks_mean":   dann_ks_mean,  "cv_ks_std":  dann_ks_std,
        "ablation":     dann_ablation,
        "holdout_main": dann_ablation[2],   # Case 3 = Full DANN
    }

LOG3.close()
RUN_LOG.write(f"[Done] PHẦN 3 — DANN (TH1 + TH2) → {OUTPUT_DIR}/03_dann.txt")



# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║            FINAL — SO SÁNH BA PHƯƠNG PHÁP & KẾT LUẬN (2 TH)                  ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
LOG_F = TeeLogger(OUTPUT_DIR / "99_final_comparison.txt")
LOG_F.section("FINAL — SO SÁNH BASELINE / HFA / DANN (DCCC hold-out) — 2 TH", level=1)

# Lấy XGBoost làm đại diện baseline mạnh nhất
...
for th_name in lc_variants:
    th_label = lc_variants[th_name]["label"]
    hfa_ho  = hfa_all_results[th_name]["holdout_main"]
    dann_ho = dann_all_results[th_name]["holdout_main"]

    LOG_F.section(f"══ {th_label} ══", level=2)
    ...
    LOG_F.section(f"Negative Transfer [{th_name}]", level=2)
    b_auc    = best_baseline["AUC"]
    hfa_auc  = hfa_ho["AUC"]
    dann_auc = dann_ho["AUC"]
    ...
    LOG_F.write("")
    if hfa_auc < b_auc:
        LOG_F.write("  HFA dưới baseline → có dấu hiệu Negative Transfer.")
    else:
        LOG_F.write("  HFA vượt baseline → transfer có ích.")
    if dann_auc < b_auc:
        LOG_F.write("  DANN dưới baseline → có dấu hiệu Negative Transfer.")
    else:
        LOG_F.write("  DANN vượt baseline → transfer có ích.")

# ── So sánh TH1 vs TH2 ──────────────────────────────────────────────────────
LOG_F.section("SO SÁNH TH1 (full features) vs TH2 (selected features)", level=1)
LOG_F.write("")
hdr2 = (f"  {'Method':<28s} {'TH1 AUC':>10s} {'TH2 AUC':>10s} {'Delta':>10s}")
LOG_F.write(hdr2); LOG_F.write("  " + "-" * 62)

hfa_th1  = hfa_all_results["TH1_full"]["holdout_main"]["AUC"]
hfa_th2  = hfa_all_results["TH2_selected"]["holdout_main"]["AUC"]
dann_th1 = dann_all_results["TH1_full"]["holdout_main"]["AUC"]
dann_th2 = dann_all_results["TH2_selected"]["holdout_main"]["AUC"]

LOG_F.write(f"  {'HFA  (Case 3)':<28s} {hfa_th1:>10.4f} {hfa_th2:>10.4f} "
            f"{(hfa_th2-hfa_th1)*100:>+10.2f}pp")
LOG_F.write(f"  {'DANN (Case 3)':<28s} {dann_th1:>10.4f} {dann_th2:>10.4f} "
            f"{(dann_th2-dann_th1)*100:>+10.2f}pp")

LOG_F.write("")
if hfa_th2 > hfa_th1:
    LOG_F.write("  → TH2 tốt hơn TH1 cho HFA: chọn lọc features giúp giảm noise.")
else:
    LOG_F.write("  → TH1 tốt hơn TH2 cho HFA: full features cung cấp nhiều thông tin hơn.")
if dann_th2 > dann_th1:
    LOG_F.write("  → TH2 tốt hơn TH1 cho DANN: selected features giảm domain gap.")
else:
    LOG_F.write("  → TH1 tốt hơn TH2 cho DANN: encoder có thể tận dụng toàn bộ features.")

# ── CV comparison ────────────────────────────────────────────────────────────
LOG_F.section("5-fold CV comparison (AUC mean +/- std)", level=2)
for method, all_res in [("HFA", hfa_all_results), ("DANN", dann_all_results)]:
    for th_name in all_res:
        r = all_res[th_name]
        cv_line = (f"  {method:5s} [{th_name:12s}]: "
                   f"AUC={r['cv_auc_mean']:.4f}+/-{r['cv_auc_std']:.4f}  "
                   f"KS={r['cv_ks_mean']:.4f}+/-{r['cv_ks_std']:.4f}")
        LOG_F.write(cv_line)

LOG_F.section("Ket luan", level=2)
LOG_F.write(
    "v10 mo rong v9: chia LC source thanh 2 truong hop (TH).\n"
    "TH1: Full features + improved imputation (median + missing indicator).\n"
    "TH2: Selected features theo 5 Cs of Credit (Altman & Saunders 1997).\n"
    "Ca HFA va DANN deu chay tren TH1 va TH2 voi cung giao thuc danh gia\n"
    "(5-fold CV + hold-out + 3-case ablation).")
LOG_F.close()
